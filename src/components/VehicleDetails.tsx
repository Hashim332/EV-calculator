...
const VehicleDetails: React.FC<VehicleDetailsProps> = ({ vehicle, type }) => {
  ...
  const resaleRatio = Number(vehicle.depreciation_3yr_percent) || 0;
  const resale = Math.round(purchase * resaleRatio);
  ...
  return (
    <>
      ...
      <ul className='text-base text-gray-700 space-y-1 mb-4'>{costDetails}</ul>
      <div
        className={`w-full rounded p-3 text-center mt-auto ${
